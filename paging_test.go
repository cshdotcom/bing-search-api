...
// paging_test.go — 翻页机制测试(v1.4.1 + v1.4.2 + v1.4.3 修复):
...
//      (v1.4.2:聚合翻页路径同样校验;v1.4.3:细分两档——首轮被拦=窗口
//      整体不可达→硬报错 429;聚合补齐被拦=窗口与第 1 页相交→
//      返回部分结果 + limited 标记,不再全盘报错)
...
func TestSearchPagedFirstPageNoFirstParam(t *testing.T) {
	...
	res, _, _, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 0, Count: 10})
	if err != nil {
		t.Fatal(err)
	}
	if len(res) != 3 {
...
func TestSearchPagedOffsetFollowsBingNextLink(t *testing.T) {
	...
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, total, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 5, Count: 2})
	if err != nil {
		t.Fatal(err)
	}
	if total != 9000 {
...
func TestSearchPagedDeepOffsetDirectFirst(t *testing.T) {
	...
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, _, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 20, Count: 3})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSearchPagedCountAggregation(t *testing.T) {
	...
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, _, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 0, Count: 8})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSearchPagedDedupAcrossPages(t *testing.T) {
	...
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, _, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 0, Count: 50})
	if err != nil {
		t.Fatal(err)
	}
	if len(res) != 3 {
...
// serpPageN 构造第 1 页 SERP,n 条结果 + 当前页标记 + 下一页链接。
// 用于复现家宽实测场景:cn.bing.com 每页只给 9 条,而 count=10 需要跨页补齐。
func serpPageN(query string, n int) string {
	var b strings.Builder
	b.WriteString(`<ol id="b_results">`)
	for i := 1; i <= n; i++ {
		fmt.Fprintf(&b, `<li class="b_algo"><h2><a href="https://example.com/%d">Result %d</a></h2>`+
	...
	b.WriteString(`</ol><span class="sb_count">About 9,000 results</span>`)
	b.WriteString(`<li class="b_pag"><nav role="navigation"><ul class="sb_pagF">`)
	b.WriteString(`<li><a aria-label="Page 1" class="sb_pagS sb_bp">1</a></li>`)
	b.WriteString(`<li><a class="b_widePag sb_bp" aria-label="Page 2" href="/search?count=10&amp;q=golang&amp;FPIG=T&amp;first=10&amp;FORM=PERE">2</a></li>`)
	b.WriteString(`</ul></nav></li>`)
	return b.String()
}

func TestSearchPagedAggregationBlockedPartial(t *testing.T) {
	// v1.4.3 语义:offset=0&count=50,首轮第 1 页正常,聚合翻页(first=10)
	// 被回吐 sb_pagS=1 → 窗口与第 1 页相交 → 返回部分结果 + limited,
	// 不再硬报错(否则“每页 9 条”的上游下连第 1 页都 429)
	ts, _ := newPagingTestServer(t, map[int]string{
		1: serpPage("golang", 1, true),
	})
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, _, limited, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 0, Count: 50})
	if err != nil {
		t.Fatalf("聚合翻页被拦时窗口与第 1 页相交应返回部分结果, got %v", err)
	}
	if !limited {
		t.Fatalf("应标记 limited=true")
	}
	if len(res) != 3 {
		t.Fatalf("应返回第 1 页的 3 条, got %v", urlsOf(res))
	}
}

func TestSearchPagedShortPagePartial(t *testing.T) {
	// 家宽实测场景(2026-09):cn.bing.com 每页仅 9 条且翻页被拦,
	// count=10 的第 1 页请求凑不齐 → 必须返回 9 条部分结果而非 429
	ts, _ := newPagingTestServer(t, map[int]string{
		1: serpPageN("golang", 9),
	})
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, _, limited, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 0, Count: 10})
	if err != nil {
		t.Fatalf("短页+翻页被拦时第 1 页应返回部分结果, got %v", err)
	}
	if !limited || len(res) != 9 {
		t.Fatalf("want 9 条 + limited=true, got %d 条 limited=%v", len(res), limited)
	}
}
...
func TestSearchPagedPagingBlocked(t *testing.T) {
	...
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	_, _, _, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 10, Count: 10})
...
func TestSearchPagedUnverifiablePageAccepted(t *testing.T) {
	...
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, _, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 10, Count: 10})
	if err != nil {
...
func TestSearchPagedFollowsAbsoluteNextLink(t *testing.T) {
	...
	e := &BingEngine{Base: ts.URL, Client: ts.Client()}
	res, _, _, _, err := e.SearchPaged(context.Background(), PagedQuery{Term: "golang", Offset: 0, Count: 6})
	if err != nil {
		t.Fatal(err)
	}
...
func TestV7SearchAggregationLimitedPartial(t *testing.T) {
	// v1.4.3:offset=0&count=50,聚合翻页被拦 → 200 + X-Paging-Limited 头 + 部分结果
	// (v1.4.2 为硬 429,导致“每页 9 条”上游下第 1 页整体不可用)
	ts, _ := newPagingTestServer(t, map[int]string{
		1: serpPage("golang", 1, true),
	})
	srv := &server{engine: &BingEngine{Base: ts.URL, Client: ts.Client()}}
	...
	if w.Code != http.StatusOK {
		t.Fatalf("部分结果应 200, got HTTP %d: %s", w.Code, w.Body.String())
	}
	if w.Header().Get(pagingLimitedHeader) == "" {
		t.Fatalf("应携带 X-Paging-Limited 头")
	}
	if got := strings.Count(w.Body.String(), `"url":"https://example.com/`); got != 3 {
		t.Fatalf("应返回第 1 页 3 条, got %d", got)
	}
}

func TestHandleSearchLimitedPartial(t *testing.T) {
	// SearXNG 网页路径同样适用部分结果语义:page=1 凑不齐 → 200 + 头
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodGet, "/search?q=golang&count=10&page=1", nil)
