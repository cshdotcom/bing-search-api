...
// 返回值多一个 limited:上游翻页受限时,已聚合到的部分结果仍随 200 返回
// (窗口与第 1 页相交时),调用方应设置 X-Paging-Limited 响应头供客户端识别。
//
...
//   - 每页抓取后校验服务端标注的当前页:被回吐第 1 页时分两种情形
//     (v1.4.3 修复,此前一律硬报错,导致“每页 9 条”的上游下连第 1 页都 429):
//     · 首轮抓取(first≥10)被拦 → 请求窗口整体在第 1 页之后,
//     无法如实服务 → 返回 ErrPagingBlocked(对外 429)
//     · 后续聚合页(page≥1)被拦 → 窗口与第 1 页相交 →
//     返回已聚合的部分结果 + limited=true(不再全盘报错)
//   - 跨页去重、按累计结果序号精确切片,页大小波动(9~10 条)不影响 offset 语义
func (e *BingEngine) SearchPaged(ctx context.Context, pq PagedQuery) ([]Result, []string, int64, bool, error) {
	want := pq.Count
	if want < 1 {
		want = 10
	}
	...
	var (
		...
		limited     bool   // 上游翻页受限:返回的是部分结果(窗口与第 1 页相交)
	)
	...
	for page := 0; page < maxFetchPages && len(results) < need; page++ {
		...
		if reqFirst >= bingPageSize {
			if served := parseServedPageNum(pageHTML); served == 1 {
				if page == 0 {
					return nil, nil, 0, false, fmt.Errorf("%w(请求 first=%d,服务端返回第 1 页)", ErrPagingBlocked, reqFirst)
				}
				limited = true
				break
			}
	...
	return results, suggestions, total, limited, nil
}

// pagingLimitedHeader 部分结果标记头:上游翻页受限时,已得结果仍随 200 返回,
// 客户端可据此识别“本页结果不全”(SearXNG / v7 两套接口通用)。
const pagingLimitedHeader = "X-Paging-Limited"
